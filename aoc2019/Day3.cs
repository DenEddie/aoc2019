...
namespace aoc2019
{
    public static class Day3
    {
        ...
        public static void Execute()
        {
            ...
            var intersections =  path1.Intersect(path2).ToList();
            Console.WriteLine($"The closest intersection is at manhattan distance: {intersections.Min(c => Math.Abs(c.Item1) + Math.Abs(c.Item2))}");

            int closestWireWize =  intersections.Min(intersection => CalculateWireDistance(path1, path2, intersection));
            Console.WriteLine($"The closest intersection is at wire distance: {closestWireWize}");
        }

        private static int CalculateWireDistance(List<Tuple<int, int>> path1, List<Tuple<int, int>> path2, Tuple<int, int> intersection)
        {
            var wireLength1 = path1.IndexOf(intersection) + 1;
            var wireLength2 = path2.IndexOf(intersection) + 1;
            return wireLength1 + wireLength2;
        }

        private static List<Tuple<int, int>> GetPath(List<string> wire)
        {
            var x = 0;
            var y = 0;
            ...
            foreach (var turn in wire)
            {
                ...
                int.TryParse(turn[1..], out int duration);

                switch (direction)
                {
                    case 'U':
                        y = GoUp(path, x, y, duration);
                        break;
                    case 'D':
                        y = GoDown(path, x, y, duration);
                        break;
                    case 'L':
                        x = GoLeft(path, x, y, duration);
                        break;
                    case 'R':
                        x = GoRight(path, x, y, duration);
                        break;
                }
            }
        ...
        private static int GoRight(List<Tuple<int, int>> path, int x, int y, int length)
        {
            for (int i = 1; i <= length; i++)
            {

                path.Add(new Tuple<int, int>(x + i, y));
            }
            return x + length;
        }
        private static int GoLeft(List<Tuple<int, int>> path, int x, int y, int length)
        {
            for (int i = 1; i <= length; i++)
            {
        ...
        private static int GoDown(List<Tuple<int, int>> path, int x, int y, int length)
        {
            for (int i = 1; i <= length; i++)
            {
                path.Add(new Tuple<int, int>(x, y + i));
            }
            return y + length;
        }
        private static int GoUp(List<Tuple<int, int>> path, int x, int y, int length)
        {
            for (int i = 1; i <= length; i++)
            {
                path.Add(new Tuple<int, int>(x, y - i));
