...
namespace aoc2019
{
    public static class Day5
    {
        ...
        {
            Console.WriteLine($"Running with 1 returns: {IntCodeComputer(1)}");
            Console.WriteLine($"Running with 5 returns: {IntCodeComputer(5)}");
        }

        private static List<int> SetInput(int input)
        {
        ...
        private static int IntCodeComputer(int input)
        {
            var program = SetInput(input);
            var instructionIndex = 0;
            var instruction = GetInstruction(program[instructionIndex]);
            while (instructionIndex < program.Count - 1 && instruction != 99)
            {
                instructionIndex = Execute(program, instructionIndex, instruction);
                instructionIndex++;
        ...
        private static int Execute(List<int> program, int instructionIndex, int instruction)
        {
            instructionIndex = instruction switch
            {
                1 => Add(program, instructionIndex),
                2 => Multiply(program, instructionIndex),
                3 => Set(program, instructionIndex),
                4 => Get(program, instructionIndex),
                5 => JumpIfTrue(program, instructionIndex),
                6 => JumpIfFalse(program, instructionIndex),
                7 => LessThanSet(program, instructionIndex),
                8 => EqualSet(program, instructionIndex),
                _ => throw new Exception("Something went wrong!!")
            };
            return instructionIndex;
        }

        private static int EqualSet(List<int> program, int index)
        {
            var parameters = GetParameters(program, index, 2).ToList();

            program[program[index + 3]] = (parameters[0] == parameters[1]) ? 1 : 0;

            return index + 3;
        }

        private static int LessThanSet(List<int> program, int index)
        {
            var parameters = GetParameters(program, index, 2).ToList();
        ...
        private static int JumpIfFalse(List<int> program, int index)
        {
            var parameters = GetParameters(program, index, 2).ToList();
            if (parameters[0] == 0)
            {
                program[index] = parameters[1];
                Execute(program, index, GetInstruction(program[index]));
            }
            return index + 2;
        }

        private static int JumpIfTrue(List<int> program, int index)
        {
            var parameters = GetParameters(program, index, 2).ToList();
            if (parameters[0] > 0)
            {
        ...
        private static int Set(List<int> program, int instructionIndex)
        {
            instructionIndex++;
            program[program[instructionIndex]] = program[program.Count - 1];
            return instructionIndex;
        }

        private static int Get(List<int> program, int instructionIndex)
        {
            instructionIndex++;
            program[program.Count - 1] = program[program[instructionIndex]];
            return instructionIndex;
        }
        ...
        {
            var parameters = GetParameters(program, index, 2).ToList();
            program[program[index + 3]] = parameters[0] * parameters[1];
            return index + 3;
        }

        private static IEnumerable<int> GetParameters(List<int> program, int index, int count)
        {
            var source = program[index] / 100;
            while (count > 0)
            {
                index++;
                var digit = source % 10 == 0 ? program[program[index]] : program[index];
                source /= 10;
                count--;
                yield return digit;
            }
