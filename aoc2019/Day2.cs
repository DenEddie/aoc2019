...
namespace aoc2019
{
    public static class Day2
    {
        ...
        {
            var part1 = CalculateOutput(12, 2);
        ...
        private static int CalculateNounAndVerb()
        {
            ...
            Parallel.For(0, 9999, (index, state) =>
            {
                var noun = index / 100;
                var verb = index % 100;
                var result = CalculateOutput(noun, verb);
                if (result == 19690720)
                {
                    realNoun = noun;
                    realVerb = verb;
        ...
        private static int CalculateOutput(int noun, int verb)
        {
            var program = SetInput(noun, verb);
            for(int i=0; i< program.Count-4; i += 4)
            {
                var result = program[i] switch
                {
